...
QVector<LogicalPlane> Camera::frustumClipPlanes() const
{
    // for example see
    // http://fgiesen.wordpress.com/2012/08/31/frustum-planes-from-the-projection-matrix/
    // http://www8.cs.umu.se/kurser/5DV051/HT12/lab/plane_extraction.pdf

    QVector<LogicalPlane> result;

    // todo - re-order these for faster rejection - eg front clipping plane
    // rejects are more likely than anything else

    QMatrix4x4 m = viewProjectionMatrix();
// left, right
    result.append( LogicalPlane( m.row( 3 ) + m.row( 0 ) ) );
    result.append( LogicalPlane( m.row( 3 ) - m.row( 0 ) ) );

// bottom, top
    result.append( LogicalPlane( m.row( 3 ) + m.row( 1 ) ) );
    result.append( LogicalPlane( m.row( 3 ) - m.row( 1 ) ) );

// front, back
    result.append( LogicalPlane( m.row( 3 ) + m.row( 2 ) ) );
    result.append( LogicalPlane( m.row( 3 ) - m.row( 2 ) ) );
...
void Camera::setStandardUniforms( const QOpenGLShaderProgramPtr& program, const QMatrix4x4& mm ) const
{
    ...
    program->setUniformValue( "modelMatrix", mm );
    program->setUniformValue( "modelNormalMatrix", mm.normalMatrix() );
}

void Camera::setStandardUniforms(const QOpenGLShaderProgramPtr &program, const QMatrix4x4 &model, const QRectF &viewport, const QPointF &p, const QSizeF &delta) const
{
    QMatrix4x4 modelViewMatrix = viewMatrix() * model;
    QMatrix3x3 normalMatrix = modelViewMatrix.normalMatrix();

    // The pickMatrix is calculated in the same way as gluPickMatrix. When used to premultiply
    // the projection matrix, it scales and translates to restrict the frustum to a size delta
    // around the picking point p in the viewport.
    QMatrix4x4 pickMatrix;
    pickMatrix.translate( (viewport.width() - 2 * (p.x() - viewport.left())) / delta.width(),
                          (viewport.height() - 2 * (p.y() - viewport.top())) / delta.height(),
                          0.0f );
    pickMatrix.scale( viewport.width() / delta.width(), viewport.height() / delta.height(), 1.0 );
    QMatrix4x4 projection = pickMatrix * projectionMatrix();

    QMatrix4x4 mvp = projection * viewMatrix() * model;
