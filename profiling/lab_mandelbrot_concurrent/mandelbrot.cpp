                             ...
                             int width, int row, const uint* scanline)
{
    ...
    for (; pixel != pixelEnd; ++pixel, ++iterationsIt) {
        ...
        for (uint i = 0; i < iterations; ++i) {
            hue += shared->histogram[i];
        }
        *pixel = generateColor(hue, shared->total);
    }
}
}
...
void drawMandelbrot(QImage *image, uint max_iterations)
{
    ...
    QThreadPool pool;
    // and we want to be fast, so let's run the code on 32 threads in parallel!
    pool.setMaxThreadCount(32);

    // step 1: run the first pass of our algorithm
    std::vector<QFuture<void> > futures(data.height);
    for (int y = 0; y < data.height; ++y) {
        futures[y] = QtConcurrent::run(&pool, firstPassMandelbrotRow,
    ...
    for (int y = 0; y < data.height; ++y) {
        secondPassMandelbrotRow(image, &shared, data.width, y, scanline(y));
    }
